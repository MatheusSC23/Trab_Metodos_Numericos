...
void strutil::split(string text, char s, string pieces[], const int piecesLen) {
    ...
    if (splitPoints != piecesLen) {
        throw std::domain_error("Passed pieces length argument do not match with the reality");
    }

    int start = 0, end = start, i = 0;
    while (end <= textLen) {
        if (end == textLen || text[end] == s) {
            pieces[i] = text.substr(start, end);
            end++;
            start = end;
            i++;
        }
